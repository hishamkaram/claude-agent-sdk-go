...
thresholds. CLI `--help` probes run without credentials. Session-backed checks
can require authentication even when they spend no model tokens. Paid runs have
test-specific opt-ins: ordinary turn tests use `CLAUDE_SDK_RUN_TURNS=1`; the
workflow test uses `AGENTD_CLAUDE_CLI_INTEGRATION=1` and
`CLAUDE_WORKFLOW_TEST_MAX_BUDGET_USD`. Inspect the selected tests before running
them with credentials. Passing mock or unit tests does not prove a live workflow.
